...
coverage_collector() {
  ...
  while [ ! -f "$COVERAGE_COLLECTOR_FINISH" ]; do
    PROFRAWS=$(find "$BUILD_ROOT" -name '*.profraw*' -mmin 30; \
      find "$YB_SRC_ROOT/java" -name '*.profraw*' -mmin 30)
    if [ -n "$PROFRAWS" ]; then
      /opt/yb-build/llvm/yb-llvm-v12.0.1-yb-1-1633099823-bdb147e6-centos7-x86_64/bin/llvm-profdata \
  ...
  echo "C: $COVERAGE_COLLECTOR_FINISH"
...
generate_coverage_report() {
  ...
  COMMAND="$0 $*"
  /opt/yb-build/llvm/yb-llvm-v12.0.1-yb-1-1633099823-bdb147e6-centos7-x86_64/bin/llvm-cov \
    show $OBJS \
